...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password'])) {
    ...
    if (strlen($password) > 20) {
        echo "<script>
                ...
                window.location.href = 'users-form.html'; // Redirect to users_form.html after alert
              </script>";
    } else {
        ...
        if (pg_num_rows($check_result) > 0) {
            ...
            if ($result) {
                ...
                echo "<script>
                        ...
                        window.location.href = 'users-form.html'; // Redirect to users_form.html after success
                      </script>";
            } else {
                // Show error message and redirect back
                echo "<script>
                        alert('Error: " . pg_last_error($conn) . "');
                        window.location.href = 'users-form.html'; // Redirect to users_form.html after error
                      </script>";
            }
        }
    }
